...
class Knight:
    ...
    def check_moves(self):
      cycle_direction = random.choice(['forward', 'backward'])  # Déterminer la direction du cycle une fois
      for i in range(len(self.chromosome.genes)):
          ...
          if not self.is_valid_move(next_position):
              ...
              # Essayer de trouver un mouvement valide en cyclant
              ...
              if not found_valid_move:
                  # Aucun mouvement valide trouvé, conserver le mouvement invalide
                  pass
          else:
              # Mouvement valide, mettre à jour la position et le chemin
              self.position = next_position
              self.path.append(self.position)
